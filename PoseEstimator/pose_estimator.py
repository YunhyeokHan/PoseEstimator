...
def plotXYZ(img, rvec, tvec, newcameramatrix):
    ...
    img = cv2.line(
        img,
        tuple(imgpts[0].ravel().astype(int)),
        tuple(imgpts[1].ravel().astype(int)),
        color=(0, 0, 255),
        thickness=3,
    )
    img = cv2.line(
        img,
        tuple(imgpts[0].ravel().astype(int)),
        tuple(imgpts[2].ravel().astype(int)),
        color=(0, 255, 0),
        thickness=3,
    )
    img = cv2.line(
        img,
        tuple(imgpts[0].ravel().astype(int)),
        tuple(imgpts[3].ravel().astype(int)),
        color=(255, 0, 0),
        thickness=3,
    )
...
def rotationMatrixToEulerAngles(R):
    ...
    if abs(R[2, 0]) != 1:
        ...
        pitch2 = math.atan2(
            R[2, 1] / math.cos(yaw2), R[2, 2] / math.cos(yaw2)
        )  # Pitch (X-axis rotation)

        roll2 = -math.atan2(
            R[1, 0] / math.cos(yaw2), R[0, 0] / math.cos(yaw2)
...
def load_calibration_data(filepath, img_size):
    ...
    mapx, mapy = cv2.initUndistortRectifyMap(
        mtx, dist, None, newcameramtx, img_size, cv2.CV_32FC1
    )
    return {
        "mtx": mtx,
        "dist": dist,
...
def plot_to_image(rx, ry, rz, tx, ty, tz, ind_range):
    ...
    for data, ax in zip([rx, ry, rz, tx, ty, tz], axes.flat):
        ...
        ax.set_ylim(
            min(data[-ind_range:]) if data else 0, max(data[-ind_range:]) if data else 1
        )
...
def process_camera_feed(
    camera, calib_data, objp, checkerboard, ind_range, update_interval
):
    """
    ...
    while camera.IsGrabbing() and is_running:
        ...
        if grabResult.GrabSucceeded():
            ...
            dst = cv2.remap(
                img, calib_data["mapx"], calib_data["mapy"], cv2.INTER_LINEAR
            )
            dst_color = cv2.cvtColor(dst, cv2.COLOR_GRAY2BGR)
            ret, corners = cv2.findChessboardCorners(
                dst, checkerboard, cv2.CALIB_CB_FAST_CHECK
            )
            if ret:
                ...
                ret2, rvecs, tvecs = cv2.solvePnP(
                    objp, corners2, calib_data["newcameramtx"], None
                )
                if ret2:
                ...
                if frame_count % update_interval == 0:
                    ...
                    dst_color = plotXYZ(
                        dst_color, rvecs, tvecs, calib_data["newcameramtx"]
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Camera pose estimator using a checkerboard."
    )
    parser.add_argument(
        "--checkerboard",
        type=int,
    ...
    calib_data = load_calibration_data(
        "PoseEstimator/calib.npz", img_size=(camera.Width.Value, camera.Height.Value)
    )

    objp = generate_objp(checkerboard, scale=square_size)
    try:
        process_camera_feed(
            camera, calib_data, objp, checkerboard, ind_range=50, update_interval=1
        )
    finally:
        camera.Close()
